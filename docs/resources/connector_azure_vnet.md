...
### VNET Peering Mode with Azure CXP

When tenant has Azure VNET 3rd party support enabled, when using `connection_mode` of `VNET_PEERING` with an Azure CXP, it is recommended to fill in  `peering_gateway_cxp_id`:
...
- `subnet_cidrs` (Set of String) Every address prefix of the VNET subnet, for a subnet that has more than one. A subnet is onboarded all or nothing, so this must name every prefix the subnet has or the request is rejected. A subnet with a single prefix may be written either way - a one-element `subnet_cidrs` is sent exactly as `subnet_cidr` would be. Requires the multi-prefix feature to be enabled on the tenant. Exactly one of `subnet_cidr` and `subnet_cidrs` is required.
- `udr_list_ids` (Set of Number) User defined routes list (`list_udr`).
