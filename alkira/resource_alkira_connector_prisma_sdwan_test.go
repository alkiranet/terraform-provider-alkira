...
func TestPrismaSDWANDeflateInstances(t *testing.T) {
	...
	for i, inst := range result {
		...
		assert.Equal(t, json.Number(strconv.Itoa(i+1)), inst.Id)
	}
}

func TestPrismaSDWANAllowListSchema(t *testing.T) {
...
func TestPrismaSDWANAllowListValidator(t *testing.T) {
	validate := resourceAlkiraConnectorPrismaSDWAN().Schema["allow_list"].Elem.(*schema.Schema).ValidateFunc
...
func TestPrismaSDWANAllowListExpand(t *testing.T) {
